# Clonar todas las notificaciones de la entidad raíz que contienen la cadena 'TICGAL' en su nombre en las demás entidades existentes en GLPI.
...
def connect_to_database():
    try:
        ...
        if connection.is_connected():
            ...
            # Obtener todas las notificaciones de la entidad raíz cuyo nombre comience con "TICGAL"
            cursor.execute("""
                SELECT * FROM glpi_notifications 
                WHERE entities_id = %s AND name LIKE 'TICGAL%';
            """, (root_entity_id,))
